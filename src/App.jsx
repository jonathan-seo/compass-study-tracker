...
const STAGES = [
  { id: 'planning', name: 'Team Planning', icon: Calendar },
  { id: 'approval', name: 'Review & Approval', icon: ClipboardCheck },
  { id: 'sourcing', name: 'Sourcing & Creation', icon: Package },
  { id: 'promotion', name: 'Promotion', icon: Megaphone },
  { id: 'active', name: 'Active Study', icon: BookOpen },
  { id: 'review', name: 'Post-Study Review', icon: Users },
];
  ...
  const StudyCard = ({ study, compact = false }) => {
    ...
    return (
      <div 
        ...
        className={`group bg-white border rounded-xl shadow-sm hover:shadow-md transition-all cursor-grab active:cursor-grabbing ${compact ? 'p-3' : 'p-5'} ${isDragging ? 'opacity-40' : 'opacity-100'}`}
      >
        <div className="flex justify-between items-start mb-2">
          ...
          <button onClick={() => handleOpenModal(study)} className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-blue-600"><MoreVertical size={16}/></button>
        </div>
        <h3 className={`font-bold text-slate-800 leading-snug mb-2 ${compact ? 'text-xs' : 'text-sm'}`}>{study.title}</h3>
        {!compact && study.notes && <div className="bg-slate-50 p-3 rounded-xl border text-xs text-slate-600 mb-3 italic">{study.notes}</div>}
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <header className="bg-white border-b px-8 py-4 sticky top-0 z-30 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800">Ministry Study Tracker</h1>
            <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">Compass Community Church</p>
          </div>
        </div>
        <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-xl border">
          <button onClick={() => setViewMode('detail')} className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold ${viewMode === 'detail' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}><LayoutGrid size={14} /> Detail</button>
          <button onClick={() => setViewMode('pipeline')} className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold ${viewMode === 'pipeline' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}><Columns size={14} /> Pipeline</button>
        </div>
        <button onClick={() => handleOpenModal()} className="bg-slate-900 text-white px-5 py-2 rounded-xl font-bold text-sm flex items-center gap-2 active:scale-95 transition-all shadow-lg shadow-slate-200"><Plus size={18}/> New Study</button>
      </header>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <aside className="lg:w-64 space-y-6">
              <nav className="space-y-1">
                {STAGES.map(s => (
                  <button key={s.id} onClick={() => setActiveStage(s.id)} className={`w-full flex items-center justify-between p-3 rounded-xl ${activeStage === s.id ? 'bg-blue-600 text-white shadow-md' : 'text-slate-600 hover:bg-white'}`}>
                    <div className="flex items-center gap-3"><s.icon size={18} /> <span className="text-sm font-bold">{s.name}</span></div>
                    <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-slate-200 text-slate-500">{studiesByStage[s.id]?.length || 0}</span>
                  </button>
                ))}
              </nav>
            </aside>
            <section className="flex-1 grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-6">
          ...
          <div className="flex gap-6 overflow-x-auto pb-6 scrollbar-hide h-[calc(100vh-140px)]">
            {STAGES.map(stage => (
              <div 
                ...
                className={`w-80 flex-shrink-0 flex flex-col rounded-2xl transition-colors ${dragOverStage === stage.id ? 'bg-blue-50 ring-2 ring-blue-300 ring-inset' : ''}`}
              >
                <div className="flex items-center justify-between mb-4 bg-slate-200/50 p-3 rounded-xl border">
                  <h3 className="text-[11px] font-black text-slate-600 uppercase tracking-widest">{stage.name}</h3>
                  <span className="text-[10px] font-black bg-white px-2 py-0.5 rounded-lg border">{studiesByStage[stage.id]?.length || 0}</span>
                </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <form onSubmit={handleSave} className="bg-white rounded-[2rem] w-full max-w-xl shadow-2xl overflow-hidden p-8 space-y-5">
            ...
            <input required className="w-full bg-slate-50 border p-3 rounded-xl font-bold" placeholder="Study Title" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
            <div className="grid grid-cols-2 gap-4">
              <select className="w-full bg-slate-50 border p-3 rounded-xl font-bold text-sm" value={formData.ministryId} onChange={e => setFormData({...formData, ministryId: e.target.value})}>
                {Object.values(MINISTRIES).map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
              </select>
              <select className="w-full bg-slate-50 border p-3 rounded-xl font-bold text-sm" value={formData.stage} onChange={e => setFormData({...formData, stage: e.target.value})}>
                {STAGES.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
            <textarea rows={3} className="w-full bg-slate-50 border p-3 rounded-xl text-sm" placeholder="Notes..." value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} />
            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 font-bold text-slate-400">Cancel</button>
              <button type="submit" className="bg-blue-600 text-white px-10 py-3 rounded-2xl font-black shadow-xl shadow-blue-100 flex items-center gap-2"><Save size={18}/> Save</button>
            </div>
          </form>
        </div>
      )}
